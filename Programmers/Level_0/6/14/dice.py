...
def solution(a, b, c, d):
    # 4개의 주사위 값을 리스트로 묶고 오름차순으로 정렬합니다.
    dice = [a, b, c, d]
    dice.sort()
    
    # 1. 네 주사위 숫자가 모두 같은 경우 [p, p, p, p]
    if dice[0] == dice[3]:
        return 1111 * dice[0]
    ...
    # 3. 주사위가 두 개씩 같은 값인 경우 [p, p, q, q]
    elif dice[0] == dice[1] and dice[2] == dice[3]:
        p = dice[0]
        q = dice[2]
        return (p + q) * abs(p - q) # abs()는 절대값을 구하는 내장 함수입니다.
        
    # 4. 어느 두 주사위만 같고 나머지 두 개는 서로 다른 경우
    # 패턴 A: [p, p, q, r]
    elif dice[0] == dice[1]:
    ...
    # 5. 네 주사위 숫자가 모두 다른 경우
        ...
        return dice[0] # 정렬되어 있으므로 가장 첫 번째 방이 최솟값입니다.
